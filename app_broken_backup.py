...
with tab4:
    ...
    # Add instructions for music
    if play_music or play_lords_prayer:
        st.info("🎵 **Note**: Music will start automatically when you begin the prayer timer. If your browser blocks autoplay, you may need to click play manually when the timer starts.")
    ...
    if start_countdown or start_with_music:
        ...
        # SIMPLE MUSIC APPROACH: Create audio ONCE and use pure JavaScript timer
        if start_with_music:
            # Create the audio elements once
            if play_music and sound_file:
                st.audio(sound_file, format="audio/mp3", autoplay=True, loop=True)
            if play_lords_prayer:
                st.audio("audio_The_Lords_Prayer.mp3", format="audio/mp3", autoplay=True, loop=True)
            
            # Pure JavaScript timer to stop after duration
            st.components.v1.html(f"""
            <script>
            console.log("🎵 Setting {duration_seconds}s timer to auto-stop music");
            
            setTimeout(function() {{
                console.log("⏰ STOPPING ALL MUSIC NOW");
            ...
            console.log("✅ Music timer set successfully");
            </script>
            """, height=0)
        ...
        for t in range(duration_seconds, -1, -1):
            ...
            # SIMPLIFIED: Just show status, music controlled by JavaScript timer above
            if start_with_music:
                status = f"🎵 Music playing - stops in {mins}:{secs:02d}" if t > 0 else "🔔 Timer complete - music stopped"
                color = "#4CAF50" if t > 0 else "#ff6b6b"
                music_placeholder.markdown(
                    f'<div style="position: fixed; bottom: 20px; left: 20px; background: rgba(0,0,0,0.8); padding: 10px; border-radius: 5px; color: {color}; font-size: 14px;">{status}</div>',
                    unsafe_allow_html=True
                )
        ...
        # Prayer timer complete
                    if t > 0:
                        ...
                        music_html += f'''
                        <audio controls loop id="prayer-music" autoplay style="width: 280px;">
                        ...
                        <script>
                        const prayerAudio = document.getElementById('prayer-music');
                        if (prayerAudio) {{
                            prayerAudio.pause();
                ...
                if play_lords_prayer:
                    if t > 0:
                        ...
                        music_html += '''
                        <audio controls loop id="lords-prayer" autoplay style="width: 280px;">
                        ...
                        <script>
                        const lordsAudio = document.getElementById('lords-prayer');
                        if (lordsAudio) {
                            lordsAudio.pause();
                ...
                if t > 0:
                    music_html += '<p style="color: #ccc; margin: 10px 0 0 0; font-size: 12px;">🎵 Music will auto-start and stop automatically</p>'
                else:
                    music_html += '<p style="color: #ffaa00; margin: 10px 0 0 0; font-size: 12px;">🔔 Prayer time complete - all music stopped</p>'
        ...
        # Final music stop - execute JavaScript to ensure all audio is stopped
        st.components.v1.html("""
        <script>
        // Final stop for all audio elements
        function stopAllAudio() {
            const allAudio = document.querySelectorAll('audio');
            allAudio.forEach(audio => {
                audio.pause();
                audio.currentTime = 0;
                audio.volume = 0; // Mute as backup
                console.log('Final stop executed for audio element');
            });
            
            // Stop by specific IDs as well
            const specificAudios = ['prayer-music', 'lords-prayer'];
            specificAudios.forEach(id => {
                const element = document.getElementById(id);
                if (element) {
                    element.pause();
                    element.currentTime = 0;
                    element.volume = 0;
                    console.log('Stopped audio by ID:', id);
                }
            });
        }
        
        // Execute immediately and retry a few times to ensure it works
        stopAllAudio();
        setTimeout(stopAllAudio, 100);
        setTimeout(stopAllAudio, 500);
        setTimeout(stopAllAudio, 1000);
        </script>
        """, height=0)
